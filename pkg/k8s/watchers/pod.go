...
func (k *K8sWatcher) updateK8sPodV1(oldK8sPod, newK8sPod *slim_corev1.Pod) error {
	...
	if labelsChanged {
		...
		err = updateCiliumEndpointLabels(podEP, newPodLabels)
		if err != nil {
			log.WithError(err).Debugf("Error while updating CiliumEndpoint object with new Pod labels")
			return err
		}
	}
...
// updateCiliumEndpointLabels update the Labels in CiliumEndpoint object by mirroring
// the labels of the associated Pod.
func updateCiliumEndpointLabels(ep *endpoint.Endpoint, labels map[string]string) error {
	...
	default:
		...
		ep.GetCiliumEndpointStatus(option.Config).DeepCopyInto(&status)
		_, err := ciliumClient.CiliumEndpoints(pod.GetNamespace()).Update(context.TODO(), &v2.CiliumEndpoint{
